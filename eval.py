...
def vis_attentions(savepath, attentions, gts, predictions):
    '''
    vis attention matrix of shape (m, L, L), but we only vis the first batch data
    :param savepath:
    :param attentions: of shape (m * nb, bl, bl)
    :param gts: ground truths (list)
    :param predictions: predictions (list)
    :return:
    '''
    nb, bl, bl = attentions.shape # assert bs = 1
    full_matrix = np.zeros((bl * nb, bl * nb))

    for i in range(nb):
        full_matrix[i*bl:(i+1)*bl, i*bl:(i+1)*bl] = attentions[i]

    fig = sns.heatmap(full_matrix, annot=False)
    heatmap = fig.get_figure()
    heatmap.savefig(savepath, dpi=400)
